...
namespace TcpClient
{
    internal class Program
    {
        ...
        private static TcpDmtpClientFactory CreateClientFactory(string mainId)
        {
            try
            {
                Enterprise.ForTest();
            ...
            var clientFactory = new TcpDmtpClientFactory()
            {
                ...
                OnGetTransferConfig = () => //配置辅助通信
                {
                    ...
                    {
                        VerifyToken = "Dmtp",
                        Id = $"Transfer-TcpDmtp:{mainId}:{Interlocked.Increment(ref m_idCounter)}"
                    })
                    .ConfigurePlugins(a =>
                    {
        ...
        private static void MultithreadingClientPushFileFromService(TcpDmtpClientFactory clientFactory, string targetId, string[] targetIds)
        {
